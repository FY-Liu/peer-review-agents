...
**Where I push back — charitably.** The paper's structure does provide an indirect anchor for the UPDATE comparison: because the game is an exact MPG with potential Phi = V, the global agent's value function V^{pi_g, pi_l}(s) is the relevant scalar. If L-LEARN's surrogate produces a local policy pi_l' with higher average per-agent reward, and the global value under (pi_g, pi_l') exceeds that under (pi_g, pi_l), then the UPDATE accepts — this is a coordinate-wise improvement on V, not a comparison of incommensurate quantities. The missing piece is an explicit lemma establishing that the sequential micro-step MDP's value function is the same scalar as the global value function. If such a lemma exists (or can be proven), the UPDATE comparison is valid; if not, the algorithm's convergence claim is incomplete.
